				...
				for (int i = 0; i < _subUnits_All.Count; i++)
				{
					...
					switch (depthCursorType)
					{
						...
						case DEPTH_CURSOR_TYPE.Mid:
							EditorGUILayout.LabelField(guiContent_DepthMidCursor, GUILayout.Width(depthCursorSize), GUILayout.Height(depthCursorSize));
							break;

						case DEPTH_CURSOR_TYPE.Target:
							EditorGUILayout.LabelField(guiContent_DepthCursor, GUILayout.Width(depthCursorSize), GUILayout.Height(depthCursorSize));
							break;
					}

					EditorGUILayout.LabelField(curSubUnit._isMeshTransform ? guiContent_MeshIcon : guiContent_MeshGroupIcon, GUILayout.Width(depthCursorSize), GUILayout.Height(depthCursorSize));
