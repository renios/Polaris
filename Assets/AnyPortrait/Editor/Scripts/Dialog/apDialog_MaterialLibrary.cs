		...
		private Shader ShaderObjectField(string label, Shader srcShader, int width)
		{
			...
			else
			{
				...
				else
				{
					...
					string strRenderPipeline = testMaterial.GetTag("RenderPipeline", false, "");

					//string strShaderKeywords = "[" + srcShader.name + "] ";
					//if(!string.IsNullOrEmpty(strRenderPipeline))
					//{
					//	strShaderKeywords += "RenderPipeline : " + strRenderPipeline;
					//}
					
					//Debug.Log(">> " + strShaderKeywords);

					if(strRenderPipeline.Contains("Lightweight"))
					{
			...
			try
			{
				...
				GUILayout.Space(5);
				if(srcShader != null && shaderType == SHADER_TYPE.LWRP)
				{
