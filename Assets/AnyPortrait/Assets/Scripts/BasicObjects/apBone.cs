			...
			if (_IKChainSet != null)
			{
					...
					)
				{
					...
					else
					{
						...
						{
							//_IKController.UpdateLookAt(_calculatedBoneIKWeight);
							if (_IKChainSet == null)
							{
								Debug.LogError("_IKChainSet is Null : " + _name);
							}
							...
							//Vector2 targetPos_Origin = _IKController._effectorBone._worldMatrix_NonModified._pos;
							//Vector2 targetPos_Cur = _IKController._effectorBone._worldMatrix._pos;
							//Vector2 endPos_Origin = _worldMatrix_NonModified._pos;
							//float length_Origin = Vector2.Distance(targetPos_Origin, endPos_Origin);
							//float length_Cur = Vector2.Distance(_worldMatrix._pos, targetPos_Cur);
							//if(length_Origin > length_Cur)
							//{
							//	length_Origin = length_Cur;
							//}
							//Vector2 expectedEndPos = targetPos_Cur + (_worldMatrix._pos - targetPos_Cur).normalized * length_Origin;

							//bool result = _IKChainSet.SimulateIK(expectedEndPos, true, true);//<<논리상 EffectorBone은 IK의 영향을 받으면 안된다.
							if (result)
							{
								IKCalculated = true;
								_IKChainSet.AdaptIKResultToBones_ByController(_calculatedBoneIKWeight);
