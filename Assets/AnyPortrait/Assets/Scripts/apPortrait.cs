...
//#endif			
		...
		//카메라 관련
		//-------------------------------------------------------------------------------------------------------
		private void CheckCameraAndBillboard()
		{
			if(_billboardType == BILLBOARD_TYPE.None)
			{
				_curCamera = null;
			...
			{
				//전체 빌보드
				//_transform.rotation = _curCamera.transform.rotation;
				_transform.rotation = _curCameraTransform.rotation;
			}
			else
			{
				//Up 고정 빌보드
				//_transform.rotation = Quaternion.LookRotation(_curCamera.transform.forward, Vector3.up);
				_transform.rotation = Quaternion.LookRotation(_curCameraTransform.forward, Vector3.up);
			}

			//여기선 Orthographic SortMode로 해야한다.
			_curCamera.transparencySortMode = TransparencySortMode.Orthographic;
			//카메라 좌표계에서의 Z값 (ZDepth)
			_zDepthOnPerspectiveCam = _curCamera.worldToCameraMatrix.MultiplyPoint3x4(_transform.position).z;
		}
